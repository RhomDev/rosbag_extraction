...
MIN_SPEED_MS  = 0.5      # seuil de mouvement (m/s)
VOXEL_SIZE    = 0.05
POISSON_DEPTH = 10
NORMAL_RADIUS = 0.2
...
def segment_ground(pcd, distance_threshold=0.2, ransac_n=3, num_iterations=1000):
    """
    Sépare le sol (plan dominant) du reste du nuage.
    Retourne: ground_pcd, nonground_pcd
    """
    import open3d as o3d
    import numpy as np
...
def main():
    ...
    print(f"[INFO] Sol      : {len(ground.points):,} points")
    print(f"[INFO] Non-sol  : {len(nonground.points):,} points")

    # ─────────────────────────────────────────────
    # MESH UNIQUEMENT SUR LE NON-SOL
    # ─────────────────────────────────────────────
    build_mesh(np.asarray(nonground.points), OUTPUT_DIR)
